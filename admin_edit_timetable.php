...
if (!$id) {
    header("Location: admin_manage_timetable.php");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_schedule'])) {
    ...
    try {
        ...
        header("Location: admin_manage_timetable.php?msg=Updated");
        exit();
    } catch (Exception $e) {
        $error = $e->getMessage();
    }
...
<div class="dashboard-container">
    <header class="dashboard-header">
        ...
        <div class="header-icons" style="margin-left: 20px;">
            <a href="admin_manage_timetable.php" style="color: #fff; text-decoration: none; font-weight: 700; font-size: 0.9rem; padding: 8px 16px; border: 1px solid rgba(255,255,255,0.3); border-radius: 12px;">Back to Master List</a>
        </div>
    </header>
    ...
    <main class="main-content" style="padding: 60px; background: #f8fafc; display: flex; justify-content: center;">
        <div style="background: #fff; padding: 50px; border-radius: 40px; box-shadow: 0 20px 60px rgba(0,0,0,0.05); width: 100%; max-width: 600px; border: 1px solid #f1f5f9;">
            ...
            <form action="" method="POST">
                ...
                <button type="submit" name="update_schedule" style="width: 100%; background: #10b981; color: #fff; padding: 18px; border: none; border-radius: 18px; font-weight: 800; cursor: pointer; font-size: 1rem; box-shadow: 0 10px 20px rgba(16, 185, 129, 0.2);">
