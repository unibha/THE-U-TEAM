...
if (!$examId) {
    header("Location: admin_manage_exam.php");
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_exam'])) {
    ...
    if (empty($exam_name) || empty($course_id) || empty($exam_date) || empty($start_time) || empty($end_time) || empty($total_marks)) {
        ...
        try {
            ...
            $message = "Examination details updated successfully! <a href='admin_manage_exam.php' style='color: #166534; font-weight: 700;'>Return to Master List</a>";
...
<div class="dashboard-container">
    <header class="dashboard-header">
        ...
        <div class="header-icons" style="margin-left: 20px; display: flex; gap: 15px; align-items: center;">
            <a href="admin_manage_exam.php" style="color: #fff; text-decoration: none; font-weight: 700; font-size: 0.9rem; padding: 8px 16px; border: 1px solid rgba(255,255,255,0.3); border-radius: 12px;"><i class="fas fa-arrow-left" style="margin-right: 8px;"></i>Back to Exams</a>
        </div>
    </header>
        ...
        <div style="max-width: 600px; margin: 0 auto;">
            ...
            <div style="background: #fff; padding: 40px; border-radius: 30px; box-shadow: 0 10px 40px rgba(0,0,0,0.03); border: 1px solid #f1f5f9;">
                <form action="" method="POST" style="display: flex; flex-direction: column; gap: 20px;">
                    ...
                    <button type="submit" name="update_exam" style="width: 100%; background: var(--brand-gradient); color: #fff; padding: 16px; border: none; border-radius: 14px; font-weight: 800; cursor: pointer; transition: 0.3s ease; margin-top: 20px; box-shadow: 0 10px 15px -3px rgba(139, 92, 246, 0.3);">
