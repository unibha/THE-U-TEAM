    ...
    if (empty($title) || empty($courseId) || empty($_FILES['resource_file']['name'])) {
        ...
        if ($uploadResult['success']) {
            try {
                ...
                header("Location: " . ROOT_URL . "/public/teacher/manage_resources.php?msg=Resource uploaded successfully!");
                exit();
            } catch (Exception $e) {
...
<div class="dashboard-container">
    <header class="dashboard-header">
        ...
        <div class="header-icons" style="margin-left: 20px; display: flex; gap: 15px; align-items: center;">
            ...
            <a href="<?php echo ROOT_URL; ?>/public/teacher/manage_resources.php" style="color: #fff; text-decoration: none; font-weight: 700; font-size: 0.9rem; padding: 8px 16px; border: 1px solid rgba(255,255,255,0.3); border-radius: 12px; background: rgba(255,255,255,0.1);">Manage Uploads</a>
        </div>
    </header>
    ...
    <main class="main-content" style="padding: 60px; background: #f8fafc;">
        <div style="max-width: 600px; margin: 0 auto; background: #fff; padding: 50px; border-radius: 35px; box-shadow: 0 20px 50px rgba(0,0,0,0.03); border: 1px solid #f1f5f9;">
            ...
            <form action="" method="POST" enctype="multipart/form-data">
                ...
                <button type="submit" style="width: 100%; background: var(--brand-gradient); color: #fff; padding: 18px; border: none; border-radius: 16px; font-size: 1.1rem; font-weight: 800; cursor: pointer; transition: 0.3s ease; box-shadow: 0 10px 25px rgba(26, 54, 93, 0.2);">
                    Publish Resource
                </button>
            </form>
        </div>
