...
/**
 * Self-healing: Ensure Teacher Attendance table exists and is migrated to support course-specific tracking
 */
try {
    $pdo->exec("
        CREATE TABLE IF NOT EXISTS teacher_attendance (
            ...
            UNIQUE KEY teacher_course_date_idx (teacher_id, course_id, attendance_date),
            FOREIGN KEY (teacher_id) REFERENCES teachers(id) ON DELETE CASCADE,
            FOREIGN KEY (course_id) REFERENCES courses(id) ON DELETE CASCADE
        ) ENGINE=InnoDB;
    ");

    // Self-healing migration: Check if course_id column exists. If not, alter the table.
    $check = $pdo->query("SHOW COLUMNS FROM teacher_attendance LIKE 'course_id'")->fetch();
    if (!$check) {
        $pdo->exec("TRUNCATE TABLE teacher_attendance");
        try {
            $pdo->exec("ALTER TABLE teacher_attendance DROP INDEX teacher_date_idx");
        } catch (PDOException $ex) {}
        try {
            $pdo->exec("ALTER TABLE teacher_attendance DROP INDEX unique_teacher_attendance");
        } catch (PDOException $ex) {}
        $pdo->exec("ALTER TABLE teacher_attendance ADD COLUMN course_id INT DEFAULT NULL AFTER teacher_id");
        $pdo->exec("ALTER TABLE teacher_attendance ADD CONSTRAINT fk_teacher_att_course FOREIGN KEY (course_id) REFERENCES courses(id) ON DELETE CASCADE");
        $pdo->exec("ALTER TABLE teacher_attendance ADD UNIQUE KEY teacher_course_date_idx (teacher_id, course_id, attendance_date)");
    }
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_status'])) {
    ...
    try {
        if ($type == 'Student') {
            ...
            ");
            $stmt->execute([$target_id, $course_id, $date, $new_status]);
        } else {
            $stmt = $pdo->prepare("
                INSERT INTO teacher_attendance (teacher_id, course_id, attendance_date, status) 
                VALUES (?, ?, ?, ?) 
                ON DUPLICATE KEY UPDATE status = VALUES(status)
            ");
            $stmt->execute([$target_id, $course_id, $date, $new_status]);
...
try {
    ...
    if ($role_filter == 'All' || $role_filter == 'Teacher') {
        $teacher_query = "
            ...
            LEFT JOIN teacher_attendance ta ON (t.id = ta.teacher_id AND c.id = ta.course_id AND ta.attendance_date = :date2)
            WHERE 1=1
        ";
...
<div class="dashboard-container">
    ...
    <main class="main-content">
        ...
        <section class="selection-bar" style="background: #f8fafc; padding: 30px; border-radius: 24px; border: 1px solid #f1f5f9; margin-bottom: 40px;">
            <form action="" method="GET" class="selection-grid-form">
                <div class="input-group">
                    ...
                    <div style="position: relative; width: 100%;">
                        ...
                        <input type="text" name="search" value="<?php echo htmlspecialchars($search); ?>" placeholder="Enter first or last name..." style="box-sizing: border-box; width: 100%; padding: 12px 12px 12px 40px; border: 2px solid #fff; border-radius: 12px; outline: none; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05);">
                    </div>
                </div>
                <div class="input-group">
                    <label style="font-size: 0.8rem; font-weight: 700; color: #64748b; margin-bottom: 8px; display: block;">Role</label>
                    <select name="role_filter" style="box-sizing: border-box; width: 100%; padding: 12px; border: 2px solid #fff; border-radius: 12px; outline: none; background: #fff; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05);">
                ...
                <div class="input-group">
                    ...
                    <select name="course_filter" style="box-sizing: border-box; width: 100%; padding: 12px; border: 2px solid #fff; border-radius: 12px; outline: none; background: #fff; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05);">
                ...
                <div class="input-group">
                    ...
                    <input type="date" name="date_filter" value="<?php echo $date; ?>" style="box-sizing: border-box; width: 100%; padding: 12px; border: 2px solid #fff; border-radius: 12px; outline: none; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05);">
                </div>
                <button type="submit" style="box-sizing: border-box; width: 100%; background: var(--brand-gradient); color: #fff; padding: 14px; border: none; border-radius: 12px; font-weight: 800; cursor: pointer;">Perform Search</button>
            </form>
        </section>
                        ...
                        <tr style="border-bottom: 1px solid #f1f5f9;">
                            ...
                            <td style="padding: 20px; text-align: center;">
                                <form action="" method="POST" style="display: flex; justify-content: center;">
                                    ...
                                    <input type="hidden" name="course_id" value="<?php echo $r['course_id']; ?>">
...
function refreshAttendance() {
    ...
    fetch(`<?php echo ROOT_URL; ?>/public/api/search_attendance.php?q=${encodeURIComponent(search)}&role=${encodeURIComponent(role)}&course=${encodeURIComponent(course)}&date=${encodeURIComponent(date)}`)
