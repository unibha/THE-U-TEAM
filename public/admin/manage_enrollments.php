...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['enroll_student'])) {
    ...
    if (empty($course_id) || empty($student_id)) {
        ...
        try {
            ...
            // 2. Update Student Class if provided
            if (!empty($class_name)) {
                $stmt = $pdo->prepare("UPDATE students SET class_name = ? WHERE id = ?");
            ...
            <div>
                <section class="selection-bar" style="background: #f8fafc; padding: 25px; border-radius: 20px; border: 1px solid #f1f5f9; margin-bottom: 35px;">
                    <form action="" method="GET" style="display: flex; gap: 20px; align-items: flex-end;">
                        ...
                        <button type="submit" style="background: var(--brand-gradient); color: #fff; padding: 12px 30px; border: none; border-radius: 12px; font-weight: 800; cursor: pointer;">Load Members</button>
                    </form>
                </section>
                ...
                <?php if ($selected_course): ?>
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px;">
                        <h2 style="font-size: 1.6rem; color: #1e293b; font-weight: 800; margin-bottom: 0;">Classroom Members</h2>
                        ...
                        <table style="width: 100%; border-collapse: collapse;">
                            <thead style="background: #f8fafc;">
                                <tr>
                                    ...
                                    <th style="padding: 20px; text-align: left; color: #64748b; font-weight: 700; text-transform: uppercase; font-size: 0.8rem; border-bottom: 2px solid #f1f5f9;">Class</th>
                            ...
                            <tbody>
                                ...
                                <?php else: ?>
                                    ...
                                    <tr style="border-bottom: 1px solid #f1f5f9;">
                                        ...
                                        <td style="padding: 20px; color: #475569; font-weight: 700;"><?php echo htmlspecialchars($s['class_name'] ?? 'N/A'); ?></td>
            ...
            <aside style="background: #f8fafc; padding: 30px; border-radius: 24px; border: 1px solid #f1f5f9; height: fit-content; position: sticky; top: 40px;">
                ...
                <form action="" method="POST">
                    ...
                    <div style="margin-bottom: 30px;">
                        <label style="font-size: 0.85rem; font-weight: 700; color: #475569; margin-bottom: 8px; display: block;">Class Assignment</label>
                        <input type="text" name="class_name" placeholder="e.g. 11, Grade 10" style="width: 100%; padding: 12px; border: 2px solid #fff; border-radius: 12px; outline: none; background: #fff; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.05);">
                    </div>
                    <?php if ($selected_course): ?>
...
if (document.getElementById('rosterSearch')) {
    document.getElementById('rosterSearch').addEventListener('input', function(e) {
        ...
        fetch(`<?php echo ROOT_URL; ?>/public/api/search_enrollments.php?q=${encodeURIComponent(query)}&course=${courseId}`)
