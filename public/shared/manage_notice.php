        ...
        <section style="display: grid; grid-template-columns: 1fr 400px; gap: 40px;">
            ...
            <aside style="background: #fff; padding: 35px; border-radius: 30px; border: 1px solid #f1f5f9; box-shadow: 0 10px 40px rgba(0,0,0,0.03); height: fit-content; position: sticky; top: 40px;">
                ...
                <form action="" method="POST">
                    ...
                    <button type="submit" name="save_notice" style="width: 100%; background: var(--brand-gradient); color: #fff; padding: 16px; border: none; border-radius: 14px; font-weight: 800; cursor: pointer; transition: 0.3s ease; box-shadow: 0 10px 15px -3px rgba(26, 54, 93, 0.3);">
                        <?php echo $editNotice ? 'Update Announcement' : 'Publish Notice'; ?>
                    </button>
                    <?php if ($editNotice): ?>
                        <a href="<?php echo ROOT_URL; ?>/public/shared/manage_notice.php" style="display: block; text-align: center; margin-top: 15px; color: #64748b; font-weight: 700; text-decoration: none; font-size: 0.9rem;">Cancel Editing</a>
                    <?php endif; ?>
                </form>
            </aside>
        </section>
...
document.getElementById('noticeSearch').addEventListener('input', function(e) {
    ...
    fetch(`<?php echo ROOT_URL; ?>/public/api/search_notices.php?q=${encodeURIComponent(query)}`)
        ...
        .then(data => {
            ...
            if (data.length === 0) {
                tbody.innerHTML = '<tr><td colspan="4" style="padding: 40px; text-align: center; color: #94a3b8; font-weight: 600;">No announcements found.</td></tr>';
                return;
            }
            ...
            data.forEach(n => {
                ...
                const pubDate = new Date(n.publish_date).toLocaleDateString('en-US', { month: 'short', day: '2-digit', year: 'numeric' });
                const prioBg = n.priority === 'Urgent' ? '#fee2e2' : '#f0fdf4';
                const prioColor = n.priority === 'Urgent' ? '#ef4444' : '#10b981';

                row.innerHTML = `
                    <td style="padding: 20px;">
                        <div style="font-weight: 700; color: #1e293b; margin-bottom: 4px;">${n.title}</div>
                        <div style="font-size: 0.8rem; color: #94a3b8; font-weight: 600;">Published: ${pubDate}</div>
                    </td>
                    <td style="padding: 20px; text-align: center;">
                        <span style="background: #f1f5f9; color: #475569; padding: 4px 12px; border-radius: 8px; font-size: 0.8rem; font-weight: 700;">${n.target_audience}</span>
                    </td>
                    <td style="padding: 20px; text-align: center;">
                        <span style="background: ${prioBg}; color: ${prioColor}; padding: 4px 12px; border-radius: 8px; font-size: 0.8rem; font-weight: 700;">
                            ${n.priority}
                        </span>
                    </td>
                    <td style="padding: 20px; text-align: center;">
                        <div style="display: flex; gap: 15px; justify-content: center;">
                            <a href="?edit_id=${n.id}" style="color: #3b82f6;"><i class="fas fa-edit"></i></a>
                            <a href="?delete_id=${n.id}" style="color: #f43f5e;" onclick="return confirm('Remove this announcement?')"><i class="fas fa-trash-alt"></i></a>
                        </div>
                    </td>
                `;
