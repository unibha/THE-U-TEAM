...
// Allow Teachers, Students, and Admins to view this page
...
// Get the requested day from URL, default to today
...
try {
    if ($role === 'Student') {
        // 1. Get the internal Student ID based on the logged-in User ID
        $stmt = $pdo->prepare("SELECT id FROM students WHERE user_id = ?");
        $stmt->execute([$userId]);
        $studentId = $stmt->fetchColumn();

        // 2. Fetch the timetable for courses this specific student is enrolled in
        $stmt = $pdo->prepare("
        ...
        // 1. Get the internal Teacher ID based on the logged-in User ID
        $stmt = $pdo->prepare("SELECT id FROM teachers WHERE user_id = ?");
        $stmt->execute([$userId]);
        $teacherId = $stmt->fetchColumn();

        // 2. Fetch the timetable ONLY for courses this teacher is assigned to
        $stmt = $pdo->prepare("
        ...
        // 1. Admin View: Fetch all scheduled classes for the selected day across the entire system
        $stmt = $pdo->prepare("
            SELECT tt.*, c.course_name, c.course_code, u.first_name as t_fname, u.last_name as t_lname
            FROM timetable tt
            JOIN courses c ON tt.course_id = c.id
    ...
    // If there is a database error, return an empty schedule
    $schedule = [];
}
?>
