...
if ($role === 'Student') {
    ...
    if (!$targetStudentId) {
        header("Location: " . ($role === 'Admin' ? ROOT_URL . '/public/admin/manage_marks.php' : ROOT_URL . '/public/teacher/manage_marks.php'));
        exit();
    }
    ...
    if ($role === 'Teacher') {
        ...
        if ($stmt->fetchColumn() == 0) {
            header("Location: " . ROOT_URL . "/public/teacher/manage_marks.php?error=Access Denied: Student not in your module.");
            exit();
        }
    }
    ...
    <header class="dashboard-header">
        ...
        <div class="header-tools">
            <div class="search-wrapper">
                ...
                <input type="text" id="marksSearch" placeholder="Search modules...">
            </div>
            <div class="header-icons" style="margin-left: 20px; display: flex; gap: 15px; align-items: center;">
                <button onclick="window.print()" class="print-btn" style="background: var(--brand-gradient); color: #fff; border: none; padding: 10px 20px; border-radius: 12px; font-weight: 800; cursor: pointer; display: flex; align-items: center; gap: 10px;">
                    <i class="fas fa-file-pdf"></i> Export Marksheet
                </button>
                <a href="<?php 
                    if($role == 'Admin') echo ROOT_URL . '/public/admin/dashboard.php';
                    elseif($role == 'Teacher') echo ROOT_URL . '/public/teacher/dashboard.php';
                    else echo ROOT_URL . '/public/student/dashboard.php';
            ...
            <section class="results-summary-grid">
                ...
                <div style="background: var(--brand-gradient); padding: 40px; border-radius: 30px; color: #fff; display: flex; justify-content: space-around; align-items: center; box-shadow: 0 15px 35px rgba(139, 92, 246, 0.25);">
                    <div style="text-align: center;">
                        ...
                        <span id="gpaDisplay" style="font-size: 2.8rem; font-weight: 900;"><?php echo number_format($overallGPA, 2); ?></span>
                    </div>
                    ...
                    <div style="text-align: center;">
                        ...
                        <span id="percentDisplay" style="font-size: 2.8rem; font-weight: 900;"><?php echo $overallPercentage; ?>%</span>
                    </div>
                </div>
            </section>
...
<script>
document.getElementById('marksSearch').addEventListener('input', function(e) {
    const query = e.target.value.toLowerCase();
    const rows = document.querySelectorAll('#resultsContainer tbody tr');
    const cards = document.querySelectorAll('.marksheet-card');
    ...
    // Hide cards if all rows inside are hidden
    cards.forEach(card => {
        const visibleRows = card.querySelectorAll('tbody tr[style="display: "";"], tbody tr:not([style*="display: none"])');
