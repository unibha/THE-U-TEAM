...
if (isset($_GET['read_all'])) {
    ...
    $stmt->execute([$myId]);
    header("Location: " . ROOT_URL . "/public/shared/notifications.php");
    exit();
}
...
if (isset($_GET['delete_id']) && $role === 'Admin') {
    ...
    header("Location: " . ROOT_URL . "/public/shared/notifications.php?msg=Deleted");
    exit();
}
...
if (isset($_GET['clear_all']) && $role === 'Admin') {
    ...
    header("Location: " . ROOT_URL . "/public/shared/notifications.php?msg=Cleared");
    exit();
}
        ...
        <div style="max-width: 800px; margin: 0 auto;">
            ...
            <?php if ($role === 'Admin' && !empty($allNotifs)): ?>
                <div style="margin-top: 40px; text-align: center;">
                    <a href="?clear_all=1" style="color: #f43f5e; font-weight: 700; text-decoration: none; font-size: 0.9rem; border: 2px solid #fecaca; padding: 10px 25px; border-radius: 12px; transition: 0.3s;" onclick="return confirm('Wipe ALL system notifications? This cannot be undone.')"><i class="fas fa-broom" style="margin-right: 8px;"></i>Clear All Notifications</a>
                </div>
            <?php endif; ?>
